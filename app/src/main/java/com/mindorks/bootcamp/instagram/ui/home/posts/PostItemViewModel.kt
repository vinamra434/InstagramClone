...
/*
    since android has view model provider only for fragment and activity so we inject item
    viewmodel by constructor injection
*/
class PostItemViewModel @Inject constructor(
    ...
    val name: LiveData<String> = Transformations.map(itemViewData) { it.creator.name }
    val postTime: LiveData<String> =
        Transformations.map(itemViewData) { TimeUtils.getTimeAgo(it.createdAt) }
    val likesCount: LiveData<Int> = Transformations.map(itemViewData) { it.likedBy?.size ?: 0 }
    val isLiked: LiveData<Boolean> = Transformations.map(itemViewData) {
        it.likedBy?.find { postUser -> postUser.id == user.id } !== null
    }

    val profileImage: LiveData<Image> = Transformations.map(itemViewData) {
    ...
    fun onLikeClick() = itemViewData.value?.let {
    ...
    fun postLikeReceived(post: Post) {
        updateData(post)
    }
